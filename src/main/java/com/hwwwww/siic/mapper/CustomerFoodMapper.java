...
public interface CustomerFoodMapper extends BaseMapper<CustomerFood> {
    @Select("select c.customer_name,c.customer_age,c.customer_sex,cf.*,f.supply_type,f.foodname " +
            "from customer c,customer_food cf,food f ${ew.customSqlSegment} " +
            " AND cf.foodid = f.id AND c.id = cf.customerid AND cf.is_deleted = 1 AND c.is_deleted = 1" +
            " AND f.is_deleted = 1")
